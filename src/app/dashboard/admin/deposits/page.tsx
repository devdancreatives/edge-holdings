...
export default function AdminDepositsPage() {
    ...
    if (loading && !data) return <div className="p-8 text-zinc-600 dark:text-zinc-400">Loading deposits...</div>
            ...
            <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody>
                            {deposits.map((d: any) => (
                                <tr key={d.id} className="border-b border-zinc-200 dark:border-zinc-800/50 hover:bg-zinc-100 dark:bg-zinc-800/20">
                                    <td className="px-6 py-4 text-zinc-600 dark:text-zinc-400">
                                        {new Date(d.createdAt).toLocaleDateString()} {new Date(d.createdAt).toLocaleTimeString()}
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-zinc-900 dark:text-white font-medium">{d.user?.fullName}</div>
